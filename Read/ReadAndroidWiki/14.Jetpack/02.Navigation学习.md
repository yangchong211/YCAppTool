...
- 03.简单实践操作
    - 3.1 第一步依赖相关库
    - 3.2 第二步创建navigation导航
    - 3.3 第三步建立NavHostFragment
    - 3.4 第四步界面跳转和传参
- 04.Navigation原理
    - 4.1 Navigation架构图
    - 4.2 Navigation源码UML
...
- 创建基础目录：
    ...
    - 创建一个Destination，如果说navigation是我们的导航工具，Destination是我们的目的地，在此之前，我已经写好了一个WelcomeFragment、LoginFragment和RegisterFragment，添加Destination的操作完成。
...
- 方式一 利用ID导航
    ...
    ```
    - LoginFragment的接收代码比较简单，直接获取Fragment中的Bundle即可，这里不再出示代码。
- 方式二 利用Safe Args
    - 目标：WelcomeFragment通过Safe Args将数据传到RegisterFragment，RegisterFragment接收后显示。
    - 看一下已经展示过的login_navigation.xml：
    ```
    <navigation>
        <fragment
            android:id="@+id/welcome">
