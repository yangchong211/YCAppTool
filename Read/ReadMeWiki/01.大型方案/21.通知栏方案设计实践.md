...
#### 2.5 通知栏三种样式
- 普通Notification：
- 折叠式Notification：折叠式Notification是一种自定义视图的Notification，用来显示长文本和一些自定义的布局的场景。它有两种状态，一种是普通状态下的视图（如果不是自定义的话和上面普通通知的视图样式一样），一种是展开状态下的视图。
...
- 创建notification必须添加的属性
    - 小图标，通过 setSmallIcon() 方法设置；标题，通过 setContentTitle() 方法设置；内容，通过 setContentText() 方法设置
