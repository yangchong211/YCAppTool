...
### 04.启动代码执行耗时
#### 4.1 启动阶段代码梳理
- 梳理从application--->splash--->main页面每个调用方法的执行耗时


#### 4.2 收集耗时信息
- 思路：通过在函数之间插桩收集函数运行数据，待首页接口数据回来后，根据插桩的数据，从中统计关键节点耗时信息，最后通过一个埋点上报。
- 耗时较长方法收集
(1) 因为每个阶段任务不一致，耗时会有很大差异，并不能以耗时多作为可优化点。
(2)关键阶段还包含很多方法，不能确定哪一个方法需要优化。因此，我们需要一个更加详细的方法耗时，帮助我们判断哪一个方法应该被优化。
