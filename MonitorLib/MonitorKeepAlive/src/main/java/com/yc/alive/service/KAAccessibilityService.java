 ...
 */
@RestrictTo(LIBRARY)
public class KAAccessibilityService extends AccessibilityService implements KAProcessCallback {
    ...
    private boolean isPackageAccessEnabled(String packageName) {
        if (KAStringUtils.isEmpty(packageName)) {
            return false;
        }
        ...
        try {
            ...
            KAExecutorUtils.getInstance().runWorker(new KAExecutorUtils.KARunnable() {
                ...
                public void runWorker() {
                    KAAppUtils.sleep(100);
                }

                @Override
                public void runUI() {
                    accessibility.process(KAAccessibilityService.this, rootNode, className.toString(), finalOption,
                        KAAccessibilityService.this);
                }
            });
        } catch (Exception e) {
            if (option != null) {
